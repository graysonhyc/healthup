...
if ($arrRecords = mysql_fetch_assoc($dbRecords)){
    if ($arrRecords["rest_lv2_score"] <= $score) {
        mysql_query("UPDATE wp_restquizscore SET `rest_lv2_score`='" . $score . "' WHERE ID ='" . $_SESSION['user_id'] . "'", $dbLocalhost);
    }
}
header ('Location: http://localhost/wordpress/level-2-rest/');
?>