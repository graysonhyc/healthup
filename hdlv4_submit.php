...
if ($arrRecords = mysql_fetch_assoc($dbRecords)){
    if ($arrRecords["hd_lv4_score"] <= $score) {
        mysql_query("UPDATE wp_hdquizscore SET `hd_lv4_score`='" . $score . "' WHERE ID ='" . $_SESSION['user_id'] . "'", $dbLocalhost);
    }
}
header ('Location: http://localhost/wordpress/level-4-handling-depression/');
?>