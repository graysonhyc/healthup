...
$_SESSION['exercises_lv4_score'] = $score;
...
if ($arrRecords = mysql_fetch_assoc($dbRecords)){
    if ($arrRecords["exercises_lv4_score"] <= $score) {
        mysql_query("UPDATE wp_exercisesquizscore SET `exercises_lv4_score`='" . $score . "' WHERE ID ='" . $_SESSION['user_id'] . "'", $dbLocalhost);
    }
}
header ('Location: http://localhost/wordpress/level-4-exercises/');
?>