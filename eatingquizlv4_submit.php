...
$_SESSION['eating_lv4_score'] = $score;
...
if ($arrRecords = mysql_fetch_assoc($dbRecords)){
    if ($arrRecords["eating_lv4_score"] <= $score) {
        mysql_query("UPDATE wp_eatingquizscore SET `eating_lv4_score`='" . $score . "' WHERE ID ='" . $_SESSION['user_id'] . "'", $dbLocalhost);
    }
}
header ('Location: http://localhost/wordpress/level-4-eating/');
